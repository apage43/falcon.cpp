    ...
    for (int il = 0; il < 1 /*TODO: replace 1 with n_layer after porting complete! */; ++il)
    {
        ...
        // self-attention
        {
            ...
            struct ggml_tensor *K =
                ggml_permute(ctx0, ggml_reshape_3d(ctx0, ggml_view_1d(ctx0, model.memory_k, (n_past + N) * head_dim, il * n_ctx * ggml_element_size(model.memory_k) * head_dim), head_dim, 1, n_past + N),
                             0, 2, 1, 3);
            ...
            struct ggml_tensor *V =
                ggml_permute(ctx0, ggml_reshape_3d(ctx0, ggml_view_1d(ctx0, model.memory_v, (n_past + N) * head_dim, il * n_ctx * ggml_element_size(model.memory_v) * head_dim), head_dim, 1, n_past + N),
                             2, 0, 1, 3);
...
int main(int argc, char **argv)
{
    ...
    for (int i = embd.size(); i < embd_inp.size() + params.n_predict; i++)
    {
        ...
        if (embd.size() > 0)
        {
            ...
            if (!falcon_eval(model, params.n_threads, n_past, embd, logits))
            { // update logits
